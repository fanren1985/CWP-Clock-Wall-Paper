...
if (versionPropsFile.exists()) {
    ...
    for (line in lines) {
        ...
        if (line.startsWith("DISPLAY_CODE=")) {
            displayCounter = line.substringAfter("DISPLAY_CODE=").trim().toIntOrNull() ?: 0
        }
    }
}
...
android {
    ...
    defaultConfig {
        ...
        // 对外显示的版本名：每次 +0.1（格式 X.X）
        versionName = "${1 + displayCounter / 10}.${displayCounter % 10}"
    }
    ...
    doLast {
        ...
        if (versionPropsFile.exists()) {
            ...
            for (line in lines) {
                ...
                if (line.startsWith("DISPLAY_CODE=")) {
                    display = line.substringAfter("DISPLAY_CODE=").trim().toIntOrNull() ?: 0
                }
            }
        }
        counter += 1
